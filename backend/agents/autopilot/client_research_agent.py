...
Usage:
    from agents.autopilot.client_research_agent import build_client_brain
...
async def build_client_brain(
    ...
    if force or not memory_store.has_entries(client_id, DESIGN_SYSTEM):
        try:
            from agents.autopilot.brand_memory import ensure_brand_memory
    ...
    if force or not memory_store.has_entries(client_id, BRAND_VOICE):
        try:
            from agents.autopilot.voice_extractor import extract_voice
    ...
    if force or not memory_store.has_entries(client_id, BUSINESS_INTEL):
        try:
            from agents.autopilot.business_researcher import research_business
