...
async def update_task_status(task_id: str, status: str = "complete") -> bool:
    """Update a ClickUp task's status.

    Args:
        task_id: ClickUp task ID
        status: Status name (e.g., 'complete', 'in progress', 'review')
    ...
    api_key = os.environ.get("CLICKUP_API_KEY", "")
    if not api_key:
        logger.warning("CLICKUP_API_KEY not set — skipping task status update")
        return False
    ...
    try:
        async with httpx.AsyncClient(timeout=30) as http:
            resp = await http.put(url, headers=_headers(), json=payload)
            resp.raise_for_status()
            return True
    except httpx.HTTPStatusError as exc:
        logger.error(
            "ClickUp update task status failed (%s, task %s): %s",
            exc.response.status_code, task_id, exc.response.text[:300],
        )
        return False
    except httpx.RequestError as exc:
        logger.error("ClickUp request error updating task %s: %s", task_id, exc)
        return False
...
async def add_task_comment(task_id: str, comment: str) -> bool:
    ...
    Args:
        task_id: ClickUp task ID
        comment: Comment text (e.g., 'Content generated by ProofPilot Agent Hub')
    ...
    api_key = os.environ.get("CLICKUP_API_KEY", "")
    if not api_key:
        logger.warning("CLICKUP_API_KEY not set — skipping task comment")
        return False
    ...
    try:
        ...
        logger.error(
            "ClickUp add comment failed (%s, task %s): %s",
            exc.response.status_code, task_id, exc.response.text[:300],
        )
        return False
    except httpx.RequestError as exc:
        logger.error("ClickUp request error adding comment to task %s: %s", task_id, exc)
        return False
